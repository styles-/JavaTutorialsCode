 ...
 *
 * <p>
 * The game begins by shuffling the deck and placing it face down. Each player
 * takes a card from the top of the deck. The player with the highest ranked
 * card (king is highest) deals four cards to each player starting with the
 * other player. The dealer then starts its turn.
 * 
 * <p>
 * The player examines its cards to determine which cards are optimal for
 * achieving four of a kind. The player then throws away one card on a discard
 * pile and picks up another card from the top of the deck. If the player has
 * four of a kind, the player puts down these cards (face up) and wins the game.
 */
public class FourOfAKind {
		...
		while (true) {
			if (curPlayer == HUMAN) {
				...
				while (choice < 'A' || choice > 'D') {
					choice = prompt("Which card do you want to throw away"
							+ " (A, B, C, D)? ");
	...
	/**
	 * Determine if the <code>Card</code> objects passed to this method all have
	 * the same rank.
	 *
	 * @param cards
	 *            array of <code>Card</code> objects passed to this method
	 *
	 ...
	 *
	 * @param msg
	 *            message to be displayed to human player
	 *
	 ...
	 *            message to be displayed to human player
	 * @param cards
	 *            array of <code>Card</code> objects to be identified
	 */
